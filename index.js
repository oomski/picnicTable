...
picnicTable.updateMatrixWorld(true);

// compute bounds and scale so the model's largest dimension equals `targetSize`
let box = new THREE.Box3().setFromObject(picnicTable);
const size = box.getSize(new THREE.Vector3());
const maxDim = Math.max(size.x, size.y, size.z);
const targetSize = 4; // world units you want the model to fit in
if (maxDim > 0) {
  const scale = targetSize / maxDim;
  picnicTable.scale.setScalar(scale);
  picnicTable.updateMatrixWorld(true); // update after scaling
}

// recompute bounds and get center
box = new THREE.Box3().setFromObject(picnicTable);
...
scene.add(pivot);
picnicTable.updateMatrixWorld(true);
picnicTable.position.sub(center); // move model so its center is at (0,0,0) relative to pivot
pivot.add(picnicTable);
